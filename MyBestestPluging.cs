...
namespace OCalcProPlugin
{
    ...
    public class Plugin : PPLPluginInterface
    {
        ...
        MRAColorsForm cForm = null;

        class MRAColorsForm : WeifenLuo.WinFormsUI.Docking.DockContent
        {
        ...
        private ToolStripMenuItem CreateMenuItemButton(string menuItemText, EventHandler handler)
        {
        ...
        {
            var cADGV = SharedReferences.cADGV;
            var cPPLMain = SharedReferences.cPPLMain;

            if (cADGV != null)
            {
                for (int i = 0; i < cADGV.Rows.Count; i++)
                {
                    if (cADGV.Rows[i].Cells["PLDBID"].Value.ToString() == cPPLMain.GetPole().GetValueString("Pole Number"))
                    {
                        var temp = sender.ToString();

                        if (temp == "Set MRA to Unset")
                        {
                            cADGV.Rows[i].Cells["MRA"].Value = "Unset";
                        }

                        if (temp == "Set MRA to Green")
                        {
                            cADGV.Rows[i].Cells["MRA"].Value = "Green";
                        }

                        if (temp == "Set MRA to Yellow")
                        {
                            cADGV.Rows[i].Cells["MRA"].Value = "Yellow";
                        }

                        if (temp == "Set MRA to Red")
                        {
                            cADGV.Rows[i].Cells["MRA"].Value = "Red";
                        }

                        if (temp == "Set MRA to Suggested")
                        {
                            cADGV.Rows[i].Cells["MRA"].Value = SharedReferences.cMyPole.SuggestedMRA;
                        }
                    }
                }
            }
        ...
        private void SetMRACheckableCell(Object sender, EventArgs e)
        {
            ...
            if (cADGV != null && cPPLMain.GetPole() != null)
            {
                for (int i = 0; i < SharedReferences.cADGV.Rows.Count; i++)
                {
                    if (cADGV.Rows[i].Cells["PLDBID"].Value.ToString() == cPPLMain.GetPole().GetValueString("Pole Number"))
                    {
                        ...
                        {
                            ChangeCellValue((DataGridViewCheckBoxCell)cADGV.Rows[i].Cells["Bkbn"]);
                        }

                        if (temp == "Check Double Wood")
                        {
                            ChangeCellValue((DataGridViewCheckBoxCell)cADGV.Rows[i].Cells["DubWood"]);
                        }

                        if (temp == "Check Span Guy")
                        {
                            ChangeCellValue((DataGridViewCheckBoxCell)cADGV.Rows[i].Cells["SpanGuy"]);
                        }

                        if (temp == "Check 3rd Party")
                        {
                            ChangeCellValue((DataGridViewCheckBoxCell)cADGV.Rows[i].Cells["Col3dPty"]);
                        }
                    }
                }
            }

            void ChangeCellValue(DataGridViewCheckBoxCell cell)
            {
